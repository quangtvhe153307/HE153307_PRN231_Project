...
using APIProject.ZaloPay.Extension;
...
namespace APIProject.ZaloPay
{
    public class ZaloPayHelper
    {
        ...
        {
            return DateTime.Now.ToString("yyMMdd") + "_" + ConfigurationUtils.Configuration["ZaloPayConfig:Appid"] + "_" + (++uid);
        }

        public static Task<Dictionary<string, object>> CreateOrder(Dictionary<string, string> orderData)
        {
            return HttpHelper.PostFormAsync(ConfigurationUtils.Configuration["ZaloPayConfig:ZaloPayApiCreateOrder"], orderData);
        }
